...
export default function Carousel({ projects = [] }) {
  ...
  const { leftIdx, centerIdx, rightIdx } = useMemo(
    ...
    <button
        ...
        className={[
        // fill parent height, prevent layout shift
        "hover:scale-105 transition duration-300 h-full w-72 sm:w-80 md:w-96 text-left bg-white rounded-3xl p-5 shadow-lg border border-black/5",
        "flex flex-col overflow-hidden", // lock content within
        "transition-all ease-in-outout focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black",
        emphasis === "center" ? "scale-100 opacity-100 blur-0" : "scale-95 opacity-60 blur-sm hover:opacity-80",
        ].join(" ")}
    >
        <h3 className="text-xl md:text-2xl font-bold text-gray-900">{p.title}</h3>

        {/* flexible body area with clamp to keep heights consistent */}
        <p className="mt-3 text-gray-700 line-clamp-5 md:line-clamp-6">
            {p.description}
        </p>
    ...
    return (
        <div className="relative w-full flex flex-col items-center">
            ...
            <div className="relative mt-6 w-full max-w-5xl h-[420px] sm:h-[460px] ">
